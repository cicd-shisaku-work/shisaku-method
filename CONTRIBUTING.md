...
- [ ] **生成される文書**：`src/` にモジュールを持つ文書は、本文を直接編集しない。モジュールを編集し、`python3 src/engine/build.py` でビルドし、**ソースと生成物を同じコミットに入れる**。
- [ ] **ビルドの検査**：`python3 src/engine/build.py --check` が通る（生成物が最新であることの機械確認）。
- [ ] **影響半径の突き合わせ**：モジュールを変更した PR は、`python3 src/engine/build.py --impact` を通し、**参照している側と突き合わせた結果を PR 説明に書く**（どこを直したか／直す必要がなかったか）。モジュールは編集の単位であって読む単位ではないため、局所の正しさが全体の矛盾になる経路は、この突き合わせでしか塞げない。
