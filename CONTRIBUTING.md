...
- [ ] **改訂の場合（境界＝ディレクトリ）**：`concepts/` 配下は本文ヘッダーの **Version と Date だけ**を更新し、**本文に改訂履歴は置かない**——履歴は git log が正本（§2 の body 義務がそのためにある。本文に埋めると同じ履歴が二箇所で育って食い違い、版を重ねるほど本文が膨らむ）。`publications/` など `concepts/` 以外は、従来どおり本文の改訂履歴に版・日付・変更点・根拠を追記する（暫定運用・境界の是非は運用で見る）。
- [ ] **自立性**：本文・図・コミットメッセージが、**本リポジトリだけを読む者に意味が通る**。ここに無い文書・リポジトリ・作業単位の名／パス／節番号に依存していない（参照は役割で書く＝§2）。判別子＝その名を消して意味が通るか。**この項だけは新規収録に限らず、改訂・修正を含むすべてのPRに掛かる**（漏れは新規追加より改訂で入る）。
